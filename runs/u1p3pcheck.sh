#!/bin/bash
echo "Cmd:       $1"
echo "Evsecon:   s0:$evsecon, s1:$evsecons1, s2:$evsecons2, lp4:$evseconlp4, lp5:$evseconlp5, lp6:$evseconlp6, lp7:$evseconlp7, lp8:$evseconlp8"
echo "Lastmgmt:  1, $lastmanagement, $lastmanagements2, $lastmanagementlp4, $lastmanagementlp5, $lastmanagementlp6, $lastmanagementlp7, $lastmanagementlp8"
echo "EvseIp:    $evseiplp1, $evseiplp2, $evseiplp3, $evseiplp4, $evseiplp5, $evseiplp6, $evseiplp7, $evseiplp8"
echo "U2p3pId:   -,$u1p3plp2id, $u1p3plp3id, $u1p3plp4id, $u1p3plp5id, $u1p3plp6id, $u1p3plp7id, $u1p3plp8id"
echo "ChargepIp: $chargep1ip, $chargep2ip, $chargep3ip, $chargep4ip, $chargep5ip, $chargep6ip, $chargep7ip, $chargep8ip"
echo "Active:    1, $u1p3plp2aktiv, $u1p3plp3aktiv, $u1p3plp4aktiv, $u1p3plp5aktiv, $u1p3plp6aktiv, $u1p3plp7aktiv, $u1p3plp8aktiv"
...
if [[ "$1" == "stop" ]]; then
	...
	if [[ $evsecon == "ipevse" ]]; then
	  ...
	  #echo "stop lastmanagement=$lastmanagement s1:$evsecons1 u1p3plp2aktiv=$u1p3plp2aktiv"
	if [[ $lastmanagement == 1 && $evsecons1 == "modbusevse" && $u1p3plp2aktiv == "1" ]]; then
		oldlls1=$(<ramdisk/llsolls1)
		echo $oldlls1 > ramdisk/tmpllsolls1
		runs/set-current.sh 0 s1
...
if [[ "$1" == "start" ]]; then
	...
	if [[ $lastmanagements2 == 1 && $evsecons2 == "extopenwb" ]]; then
		...
		mosquitto_pub -r -t openWB/set/isss/Current -h $chargep3ip -m "$oldlls2"
	fi
	if [[ $lastmanagementlp4 == 1 && $evseconlp4 == "extopenwb" ]]; then
